...
- 알림: `console.error("[WATCHDOG ALERT] …")`는 항상 남고(`wrangler tail`로 확인), `NOTIFY_WEBHOOK_URL`이 설정되면 그 주소로 `{"text": ...}` JSON을 POST한다(Slack/Discord incoming webhook 호환). 키 조회나 발송이 실패해도 값 없는 로그만 남기고 cron은 계속된다. 채널 추가는 `packages/core/src/notify.ts`의 `notifyAdmins` 한 곳에만 붙인다.
...
- 웹은 진입 화면(`/preview`)의 일반 HTML 폼을 `/preview/unlock`에 POST한다. 웹 라우트가 API로 코드를 검증하고 받은 토큰을 웹 도메인의 HttpOnly 쿠키로 심은 뒤 303으로 새 문서를 요청한다(웹·API 도메인이 달라 쿠키를 웹 쪽에서 다시 심어야 한다). 서버 액션 화면 전환을 쓰면 셸 없는 `/preview` 루트 레이아웃이 로그인 뒤에도 남아 새로고침해야 헤더·사이드바가 나타나므로 일반 POST·303이 계약이다. 이후 요청은 `middleware.ts`가 쿠키 없으면 진입 화면으로 유도하고, `api.ts`가 그 쿠키를 API로 포워딩한다. 웹은 `CCC_PREVIEW='true'` 런타임 변수로만 이 경로가 켜진다.
- 검증 통과 시 고정 데모 실무자(`PREVIEW_ACTOR_EMAIL`, 시드 users 디렉터리에 존재해야 함) 신원으로 동작한다 — 권한 모델(R1 게이트웨이)은 미리보기에서도 그대로다.

#### 기관 관리자 시점 (2026-07-30 Q 요청)
...
- 기관 관리자 전용 주소는 `/preview/admin`이다. 코드를 통과하면 `/settings`로 이동하고 `기관 관리자 설정` 구역에서 기관·배정·사용자·실무자 초대 화면으로 들어간다. 일반 미리보기 화면에도 이 주소로 가는 링크가 있다.
- **토큰 형식은 그대로다.** 세션 토큰은 발급에 쓰인 코드를 HMAC 키로 서명하므로, 리졸버가 코드별로 갈라 검증하면 신원이 갈린다 — 토큰에 역할을 적어 넣지 않는다(적으면 위조 표면이 는다).
- **코드와 이메일이 둘 다 있어야 기관 관리자 경로가 열린다.** 하나만 설정된 상태에서 열어 주면 실무자 이메일로 기관 관리자 코드가 통해 의도하지 않은 경로가 생긴다. 시크릿을 안 넣는 것이 곧 "기관 관리자 시점 없음"이다.
- **응답은 한 가지다.** 두 코드를 항상 둘 다 비교하고(일찍 빠져나오지 않는다) 실패는 401 하나로만 답한다 — 응답 시간이나 메시지로 "기관 관리자 코드가 있다"가 새지 않게 한다.
- 기관 관리자 시점도 **같은 가상 시드**를 본다. 운영 PII 와 연결되지 않는 것은 그대로다.
