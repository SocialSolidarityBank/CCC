...
### `green` 새로 만드는 시드는 낡지 않는다 / `yellow` 이미 넣은 DB 는 여전히 늙는다 (2026-07-31)

`scripts/seed/content.ts` 의 날짜가 **기준일 상대 오프셋**(`at(일수)`·`day(일수)`)으로 바뀌었다. 기준일 기본값은 시드를 만드는 날(KST)이고, `generate.ts` 의 `UPCOMING_FROM` 도 같은 기준일에서 파생한다. 그래서 **새로 만드는 시드는 언제 만들어도 '다가오는 일정'이 채워진다.** 재현이 필요하면 `SEED_ANCHOR_DATE=YYYY-MM-DD` 로 고정하고, 어느 기준일로 만들어졌는지는 `manifest.json` 의 `anchorDate` 에서 읽는다.

`yellow` **다만 이미 DB 에 들어간 일정은 재생성으로 고쳐지지 않는다.** 그쪽은 `pnpm seed:reschedule` 이 UPDATE SQL 을 만들어 주고(적용은 사람이 한다), 아래 두 규칙은 그 스크립트가 지키는 것과 같다.

**참고 — 그전 상황**: 날짜가 하드코딩 절대값이던 때는 시간이 흐르면 '다가오는 일정'이 **실무자·관리자 양쪽 모두** 비었다. 2026-07-30 에 실제로 그 상태였다(예정 16건이 전부 과거, 가장 늦은 것이 `2026-07-28`).
...
`pnpm seed:reschedule` 이 위 두 규칙(`status='scheduled'` 만 · `version+1` 동반)을 지킨 UPDATE SQL 을 만들어 낸다. 손으로 쓰지 말고 그걸 쓴다.
