...
- 상태: Accepted (2026-09-09 Q가 실행 권한의 분리 방향과 최초 관리자 초대 방식을 승인. 공급자 선정·배포·운영 검증 승인이 아님)
- 결정일: 2026-09-09
- 결정 번호: D89
...
## 2026-09-09 후속 결정: 최초 관리자 계정

Q는 Community Cloud의 최초 관리자 계정을 **설치 도구가 지정 이메일에 보내는 1회용 초대**로 만들도록 선택했다. 설치 담당자가 기관 생성자의 이메일을 입력하면 설치 도구는 설치 시점의 관리자 자격으로 해당 이메일에 Supabase Auth 초대를 보내고, 사용자는 초대 링크에서 비밀번호와 MFA를 직접 등록한다. 설치 도구는 비밀번호를 만들거나 전달하지 않으며 업무 API에는 관리자 자격을 두지 않는다.

- S11 §2.6 "설치 과정에서 기관 관리자 계정이나 실사용자 계정을 자동 생성하지 않는다"는 이 초대 방식으로 부분 대체한다. 설치 도구는 사람이 지정한 이메일 하나에만 초대를 보내며, 사람이 없는 대량 계정 생성이나 대화형 계정 생성, 시크릿 출력은 계속 금지한다.
- 초대 수락 뒤 그 Auth 사용자는 기존 `users.auth_subject`로 기관 생성자 행에 연결한다. 연결되지 않은 Auth 사용자는 업무 API actor가 되지 않는다.
- 초대 이메일은 설치 승인 정보와 설치 영수증에 hash로만 남긴다. 초대 링크, 토큰, 원문 이메일은 로그·영수증·출력에 남기지 않는다.
- 기관 생성자는 첫 로그인에서 D86의 기관 초기 설정을 완료한다. 첫 로그인 전에는 업무 API에서 초기 설정 미완료 상태를 명시적으로 응답하며 임의 성공 화면을 만들지 않는다.
- 이후 직원 초대의 관리자 자격 경계는 별도 결정이다. 업무 API에 Supabase 관리자 키를 두거나 StorageSigner에 Auth 관리자 기능을 얹지 않는다.

## 2026-09-10 후속 결정: 설치 소유권 승인

Q는 S11이 요구하는 기관·프로젝트 소유권 증거를 **설치기 전용 서명 승인서**로 분리하기로 했다.
S2 공개 signed manifest의 형식은 유지하며 기관·소유자 필드를 브라우저에 추가로 전달하지 않는다.

승인서는 기관, 프로젝트, 기대한 소유자, installation ID, 공개 manifest 전체의 해시, 계약 버전과 만료를
서명한다. 설치기는 두 서명과 결합을 검증한 뒤 실제 소유자를 대조하며, 전부 맞기 전에는 journal을
포함한 DB 변경을 하지 않는다. 승인서 생성과 갱신은 설치 도구가 처리하고, 실측 소유자를 자동으로
승인값으로 삼지 않는다. 정확한 입력과 실패 계약은 S11 §2.1이 갖는다.

공개 manifest 하나를 확장하는 대안은 기존 소비자 전부를 바꾸고 불필요한 관리 식별자를 브라우저에
전달하므로 채택하지 않았다. 분리 방식은 공개 계약을 보존하는 대신 두 문서의 연결과 갱신 검증을
설치 도구가 책임진다. 이 결정은 DB 변경 실행, 시크릿 공개, AI/STT 활성화 승인을 대신하지 않는다.
...
- S2의 이후 직원 초대에 필요한 별도 Auth 관리자 권한 경계. 최초 관리자 초대 방식은 위 후속 결정으로 확정했지만 실제 초대 발송과 외부 설치 실행은 별도다.
- STT/AI 제품 활성화, 실패한 개인정보 가림의 통과 처리, 기존 미완 기능의 제외

이후 직원 초대의 권한 경계는 후속 결정 전까지 미완료다. 이를 StorageSigner에 Auth 관리자 기능을 얹는 방식으로 해결하지 않는다. 기존 canonical 신원 연결은 `users.auth_subject`이며 문서의 다른 표기를 이유로 새 컬럼을 만들지 않는다.
