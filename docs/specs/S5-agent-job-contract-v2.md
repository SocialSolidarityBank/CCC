...
`POST /pipeline/jobs/:jobId/mask-dictionary`는 org/job/claim token/attempt에 묶인 일회성 claim-bound 응답이다. 요청 body는 정확히 `{claimToken, attempt}`이고 TLS 인증 채널에서만 받는다. `MaskDictionaryResponse`의 수명은 최초 발급 시점부터 최대 5분이며 `Cache-Control: no-store`를 설정한다. 응답은 dictionary ID, job ID, 만료 시각, `oneTime: true`와 치환 entries만 담는다. 직접 식별자 원문과 dictionary는 로그나 결과에 남기지 않는다. Agent의 메모리 폐기 관문은 [S6 §2.4](S6-privacy-packet.md#24-agent와-코어의-경계)를 따른다.

첫 발급과 재전송 모두 `mask_dictionary_read`로 감사하며 최초 발급은 consumed 시각도 기록한다. 응답 유실 뒤 같은 service/org/job/claim/attempt가 만료 전에 재전송하면 최초와 같은 dictionary ID, 만료와 응답을 반환한다. 이를 위해 원문 표를 서버 cache나 DB에 보존하지 않는다. 서버는 별도 용도로 도출한 비밀키의 검증값을 기존 opaque ID metadata에 보존하고, 현재 금고에서 재생성한 entries와 service/org/job/claim/attempt/만료가 최초 검증값과 모두 일치할 때만 반환한다. 서버 재시작이나 인스턴스 변경 자체는 재전송 실패 사유가 아니다. 등록값 변경, 검증값이나 만료 metadata 변조, 키 불일치 및 검증값 없는 기존 ID는 `dictionary_already_consumed`로 거부하고 같은 ID 아래 새 표를 반환하지 않는다.

동시 최초 발급은 기존 job CAS의 승자 응답으로 수렴한다. 복호화와 서명 뒤에도 live claim과 만료를 다시 확인한다. 다른 claim/attempt나 만료 뒤 요청은 `stale_claim`, `lease_expired` 또는 `dictionary_already_consumed`로 거부한다. dictionary 거부로 아직 live인 작업은 Agent가 `permanent/masking_failed`로 한 번 종료한다. 재생성 검증 실패를 STT 재시도로 바꾸지 않으며, 이미 서버가 종료한 result 거부에는 추가 release를 보내지 않는다.
