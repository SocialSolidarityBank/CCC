...
Cloud upload target과 Local의 직접 `put`은 **유효한 recording consent와 route-specific consent가 있고, 현재 healthy하며 이 기관에 eligible하고 capacity가 있는 Agent가 확인되는 경우에만** admission CAS를 통과한다. `local` STT route는 recording consent만 요구하고 external-STT consent가 declined여도 허용한다. `azure`/그 밖의 external route는 recording consent와 external-STT consent를 모두 요구한다. admission 실패면 Cloud upload target/Local upload를 거부하고 원음 object를 보관하지 않으며 D8 manual note를 즉시 공식 기록 경로로 제공한다. 통과한 Cloud target은 한 번만 유효하고 동일 key overwrite를 허용하지 않는다. upload completion은 session과 길이·MIME이 맞는 object를 `available`로 표시하되 client hash를 trusted hash로 승격하지 않는다. 직접 upload의 실제 bytes 검증은 첫 eligible claim의 Agent streamed re-hash가 담당한다. hash 불일치면 외부 STT provider를 호출하지 않고 영구 실패 cleanup을 시작하며 D8 수기 기록 경로를 제공한다.
...
외부 STT admission은 claim에 저장한 SG7 aggregate `ConsentGateReceipt`의 revision/effectiveness와 현재 receipt를 다시 대조하는 CAS 직후에만 허용한다. `local` route의 receipt는 recording consent만 유효하면 되고, `azure`/external route의 receipt는 recording과 external-STT consent가 모두 유효해야 한다. withdrawal CAS가 먼저 커밋되면 provider call은 0회이고, admission CAS가 먼저 커밋된 호출은 withdrawal 이후 새 호출로 간주하지 않으며 결과·삭제를 bounded residual로 조정한다. receipt revision이 없거나 stale이면 claim/start와 mint 모두 거부한다.
...
`audio_objects`는 최소 `id`, `org_id`, `session_id`, `key`, `key_hash`, `state`, `generation_id`, `stt_route`, `content_length`, `content_type`, `client_asserted_sha256`, `object_sha256`, `consent_gate_receipt_revision`, `consent_gate_receipt_effective`, `uploaded_at`, `upload_expires_at`, `retention_hard_cap_at`, `first_agent_available_at`, `processing_deadline_at`, `claim_id`, `claim_agent_id`, `claim_expires_at`, `processing_attempt_id`, `processing_started_at`, `processed_at`, `deletion_reason`, `deletion_attempt_id`, `next_attempt_at`, `retry_count`, `incident_outbox_key`, `manual_note_outbox_key`, `deleted_at`, `deletion_evidence`, `created_at`, `updated_at`을 가진다. provider idempotency/attempt audit는 S5의 result contract를 참조한다.

### 3.2 전이와 경합 규칙

| 현재 | 사건·원자 조건 | 다음 | 실패·경합 처리 |
|---|---|---|---|
| 없음 | recording consent와 route-specific consent가 유효하고 현재 healthy·eligible·capacity 있는 Agent 확인을 포함한 upload admission CAS 성공 후 intent 생성 | `pending_upload` | `local` route는 recording consent만, `azure`/external route는 recording+external-STT consent를 요구; admission 실패면 원음 object를 만들지 않고 manual-note outbox만 공식 기록 |
| `pending_upload` | target upload 완료 후 길이·MIME 검증과 upload 시점에 capture한 SG7 aggregate `ConsentGateReceipt` revision/effectiveness 재검사 | `available` | route receipt가 stale/withdrawn이면 `available`로 만들지 않고 `deletion_pending` (`consent_withdrawal` 또는 `rejected_upload`)로 즉시 cleanup; Cloud는 signed target expiry+60s까지 late/replayed bytes를 재조정 |
...
| `available` | business-day gate 후 eligible Agent가 이 object의 claim CAS에 성공하고 configured STT route/provider가 healthy+capacity이며 SG7 aggregate `ConsentGateReceipt` revision/effectiveness가 CAS에 포함됨 (`retention_hard_cap_at > now`, `processing_deadline_at IS NULL OR processing_deadline_at > now`) | `claimed` 및 `first_agent_available_at` 기록 | `local`은 recording consent receipt, `azure`/external은 recording+external-STT receipt 필요; 빈/global poll, STT off, unhealthy provider, capacity 없음, stale/withdrawn receipt, 다른 object claim 실패는 기회를 만들지 않음 |
| `claimed` | claim owner가 Agent re-hash를 성공하고 processing-start CAS (`retention_hard_cap_at > now`, `processing_deadline_at > now`, SG7 aggregate receipt revision/effectiveness current) | `processing` | hash mismatch는 `deletion_pending` (`hash_mismatch`), provider call 0회, 늦은/비소유 start 거부 |
...
| `available` | provider preflight가 permanent processing failure를 선언 | `deletion_pending` (`processing_failed`) | 즉시 delete/unlink; admin incident와 manual-note outbox를 같은 DB batch에 기록 |
| `claimed`/`processing` | permanent provider/processing failure + matching current `claimToken`/`processing_attempt_id`/consent revision·effectiveness + `now < processing_deadline_at` + `now < retention_hard_cap_at` | `deletion_pending` (`processing_failed`) | claim/lease와 new-mint authorization을 원자적으로 invalidate하고 generation-bound immediate deletion을 enqueue; admin incident와 manual-note outbox를 같은 DB batch에 기록; late worker result/release는 `stale_claim` |
...
| `processing` | successful STT/result commit과 processed deletion intent를 같은 DB batch/outbox 경계에서 기록하고, state·claim token·`processing_attempt_id`·current SG7 aggregate receipt revision/effectiveness·`now < processing_deadline_at`·`now < retention_hard_cap_at` 조건을 만족 | `deletion_pending` (`processed`) | input/hash verification만으로 deletion intent를 만들지 않음; commit 직후 delete/unlink 요청 |
...
`available` row의 다음 영업일 gate 뒤, (1) paired Agent가 이 row에 eligible하고, (2) 관리자가 명시한 STT route/engine이 configured이며 health check를 통과하고, (3) 해당 Agent에 처리 capacity가 있고, (4) SG7 aggregate `ConsentGateReceipt`의 revision/effectiveness가 route 규칙에 맞게 유효하고 claim CAS에 포함되고, (5) 이 row의 claim CAS가 성공한 경우에만 그 claim 성공 시각을 `first_agent_available_at`으로 기록한다. `local` route는 recording consent만, `azure`/external route는 recording+external-STT consent가 필요하다. global queue poll, failed claim, empty claim, STT `off`, unhealthy/missing provider, capacity 없는 Agent, stale/withdrawn receipt는 시계를 시작하지 않는다. 다른 eligible Agent가 같은 row의 claim을 먼저 성공한 경우에만 그 승자의 시각이 기회가 된다. 이 시각에 `processing_deadline_at = min(first_agent_available_at + 24h, retention_hard_cap_at)`을 한 번만 기록한다.
...
- expiry transition, claim/start, lease recovery, deletion terminal CAS는 state·claim/generation·deadline·retention cap·SG7 aggregate `ConsentGateReceipt` revision/effectiveness 조건을 포함한 단일 atomic CAS다. `local` route는 recording consent receipt, `azure`/external route는 recording+external-STT receipt가 필요하다. claim/start는 `retention_hard_cap_at > now`를 반드시 만족하고, 실제 기회가 시작된 row는 `processing_deadline_at > now`도 만족해야 한다. startup과 scheduler가 동시에 실행돼도 한 전이만 승리한다. deadline 또는 retention cap 도달 뒤 active claimed lease는 expiry를 이기지 못한다.
...
- result commit CAS는 `state=processing`, 현재 `claim_id`, `processing_attempt_id`, 현재 SG7 aggregate `ConsentGateReceipt` revision/effectiveness, `now < processing_deadline_at`, `now < retention_hard_cap_at`을 모두 조건으로 한다. route가 `local`이면 recording consent receipt만, `azure`/external이면 recording+external-STT receipt만 유효한 것으로 판정한다. just-before-deadline만 공식화할 수 있고 at/after deadline의 결과는 거부한다.
...
- [ ] Cloud private 2h upload(`upsert:false`), route-specific admission, Agent-only max 600s GET, bearer non-revocation bound, human/browser GET 금지와 Local/R2 null target·stream API가 고정되어 있다.
- [ ] versioned Korea business calendar, object-specific real opportunity predicate, route-specific ConsentGateReceipt, upload+24h 금지, first opportunity+24h limited by `retention_hard_cap_at = uploaded_at + 7*24h` UTC, 6h watchdog과 daily alert가 완결되어 있다.
...
- [ ] 아래 fixture 19종의 입력·기대 state/evidence/권한 결과와 검증 명령·실패 판정이 §6에 있다.
...
| A13 upload admission | recording consent 없음, route-specific consent 없음, healthy eligible Agent 없음, provider unhealthy/capacity 없음 | `local`은 recording consent가 있으면 external-STT declined여도 upload/claim 허용; `azure`/external은 external-STT 없으면 target/put reject; Agent/preflight 실패면 원음 object 0건과 manual-note outbox 1건 |
...
| A17 processing failure vs retry exhaustion | provider preflight permanent failure, or `claimed`/`processing` permanent failure with matching current `claimToken`/attempt/consent and both `now < processingDeadlineAt` and `now < retentionHardCapAt`; transient failures until retry budget exhausted | exact processing-failed row makes `deletion_pending(processing_failed)`, atomically invalidates claim/new-mint, and immediately enqueues generation-bound delete+incident/manual-note; transient path is `retry_exhausted`; processed result/deletion intent duplicate 0건 |
| A18 local-decline-external | `local` STT route, recording consent 유효, external-STT consent declined, healthy eligible Agent/capacity 있음 | upload admission·claim/start/result 허용; aggregate `ConsentGateReceipt` revision에 recording만 유효; external provider call 0회 |
| A19 azure-requires-external | `azure` route, recording consent 유효, external-STT consent declined 또는 stale | upload target/put·claim/mint/start reject; 원음 object 0건, manual-note outbox 1건, external provider call 0회 |

A1~A19 중 하나라도 four boolean이 fresh true가 아닌 상태에서 terminal을 만들거나, upload+24h로 A7을 삭제하거나, `uploaded_at + 7*24h` UTC cap을 넘기거나, retention cap 사유가 아닌데 full opportunity window 전에 삭제하거나, local recording-consent-only route를 거부하거나, external route에 stale/누락 external-STT consent로 진입시키거나, human/public/long URL·stale consent call·re-record CTA·provider automatic fallback이 관찰되면 suite와 `guard:audio-lifecycle`은 실패한다. 기존 assertion 삭제·skip·완화도 실패다.
