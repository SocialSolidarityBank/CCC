...
부모 트랙에 필요한 후속 문서 편집은 이 스펙에서 수행하지 않는다. S11은 PostgreSQL migration order와 `ccc_api` 생성 위치를 `0001_baseline` → `0002_sql_portability` → `0003_timestamp_normalization` → `0004_supabase_platform` → `0005_consent_six_domains` → `0006_audio_objects`로 맞추고, master plan은 E3-8 parity 경로를 SQLite `0048/0049` 및 PostgreSQL `0005`로 갱신해야 한다. SG8은 resolved commit `9125dc8`과 PostgreSQL `0006_audio_objects` 의존성을 연결해야 한다. 이 항목들은 부모가 별도 구현 커밋에서 수행한다.
