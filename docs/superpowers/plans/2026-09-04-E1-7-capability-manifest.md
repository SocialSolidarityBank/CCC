...
- Disabled reasons are deterministic: no registry entry for a STT mode → `unverified`; entry present but azure key missing → `missing_key`; entry and key present but Agent not connected → `unsupported`. `openai`: key missing → `missing_key`; key present but pilot switch, external-call switch, or Agent not ready → `unsupported`.
