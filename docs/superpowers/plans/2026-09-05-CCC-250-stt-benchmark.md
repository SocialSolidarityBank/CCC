...
- Runtime assessment: Q selected Windows for the full run and skipping the Mac full run. Native Windows 11 Pro build 26200 and AMD Ryzen 7 3700X (8 cores, 16 logical processors, approximately 24 GiB RAM) are verified. Python 3.12.13, separate CPU environments, FFmpeg 9.0.1, all 150 fixture sessions and six pinned model snapshots are prepared under a private ASCII-only path. Connectivity has returned, no surviving worker remained, and both native ASR smoke checks passed. The complete comparison is running as Windows task `CCC-E5-8-Benchmark`, independently of SSH; final gates remain unmeasured.
...
- [ ] Finish all 150 sessions on Windows CPU and preserve both candidates' complete results. Run `e5-8-windows-cpu-001` has started; no full gate has been measured.
...
- After connectivity returned, native smoke inference completed for diarization and both ASR candidates. These runtime checks do not establish accuracy or pooled RTF acceptance. The full comparison started at `2026-09-06T02:05:09.777697+00:00` from frozen checkpoint `3eb64d8`, and its first diarization row was measured. Windows task `CCC-E5-8-Benchmark` owns execution; replacing the Mac progress monitor did not restart inference. The launcher prevents idle sleep only while running and does not change the power plan.
- [Sanitized Windows preflight evidence](../../../artifacts/pilot/e5-8-windows-preflight.json) records hardware/runtime metadata, integrity checks, test counts, completed runtime smokes and launch evidence. Its progress count is a dated observation, not the current count. It is not a full STT gate or three-mode validation.
