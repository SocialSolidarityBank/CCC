...
## 26. P5 마무리와 전체 흐름 재검증 (2026-09-10)

### 이번에 추가한 것

- AI 정리 검토 화면의 `[액션으로 등록]`(D70). `next_session_commitments` 구획의 claim 만 버튼을 갖고,
  문구만 프리필한다. 담당과 기한은 실무자가 고른다. 등록은 `POST /cases/:id/action-items` 하나이고
  승인 상태를 바꾸지 않는다. 나머지 P5 항목(인테이크 위저드, 세부 목표, 불일치 3종, 종결)은 이미
  구현돼 있어 이번에는 실화면 재검증만 했다.

### 엔드포인트가 없어 만들지 않은 것

- 팀 감독 지정(P7), 동의 문안 관리(P7), 리포트(P6), 요청 링크·초대(P8), D87 잠금. §21·§23·§24 참조.

### 전체 흐름 결과 (합성 미리보기, 관리자 계정)

| # | 단계 | 결과 |
|---|---|---|
| 1 | 로그인 + 추가 인증 | 통과 |
| 2 | 기관 초기 설정과 사업 도입 확인 | 통과 |
| 3 | 당사자 등록 화면의 막힘 안내 | 통과(일반 등록은 서버 게이트가 막고, 긴급 사유 경로만 열림) |
| 4 | 당사자 목록 | 통과 |
| 5 | 당사자 정보 HERO와 여섯 영역 구획 | 통과 |
| 6 | 동의 기록(동의함 → 철회함 노출) | 통과 |
| 7 | 일정 주간 | 통과 |
| 8 | 일정 월간 7열 격자(셀 높이 148 실측) | 통과 |
| 9 | 15초 페이지 | 통과 |
| 10 | 불일치 처리 3종(기록 오류) | 통과. 처리 뒤에도 양쪽 인용 문자열 동일(원본 불변) |
| 11 | 15초 페이지에서 액션 등록 | 통과 |
| 12 | 인테이크 위저드(정본 질문지 4부) | 통과 |
| 13 | 상담 기록하기 | 통과 |
| 14 | 상담 기록 확인하기 | 통과 |
| 15 | 세부 목표 추가 | 통과 |
| 15b | 세부 목표 문구 수정 → 이전 문구 이력 노출 | 통과 |
| 15c | 세부 목표 닫기(달성·중단·재설정 3종) → `종료: 달성` | 통과 |
| 16 | AI 검토에서 약속 claim → 액션 등록 | 통과 |
| 17 | AI 초안 승인(화자 매핑 확인 뒤 승인됨) | 통과 |
| 18 | 사업 종결(사유 입력 → 종결 상태와 사유 표시) | 통과 |
| 19 | 설정 8탭(사용자·배정·보유기간·보존 검토·감사·기억·기관 정보·연결 상태) | 8/8 통과 |
| 20 | 로그아웃 | 통과 |
...
## 29. P6 전체 상담 리포트 (2026-09-10, API 레인 530bc32 소비)

- `GET /support-cases/:id/report` 하나를 읽는다(`SupportCaseReport`, schemaVersion 1).
  화면은 문장을 만들지 않고 서버가 준 저장 근거(`ReportEvidence`)만 렌더한다.
- **근거 없는 구획은 아예 그리지 않는다.** 대신 머리에 "자료가 없어 빠진 구획"을 적고
  `해당 사항이 없다는 뜻이 아니라 기록된 근거가 없다는 뜻입니다` 를 붙인다. 빈 구획을 안전 판정으로
  바꿔 읽지 않는다.
- 첫 인테이크 목표는 `firstIntakeGoal` 만 쓴다. 없으면 "기록이 없습니다" 로 두고 현재 전체 목표로
  대신 적지 않는다.
- 회차 요약이 없으면 "요약으로 쓸 근거가 없습니다" 로 두고, 근거마다 `N회차 기록 보기` 링크가
  해당 회차 앵커로 간다(D73).
- 입구는 상담 기록 확인하기 화면의 `전체 상담 리포트` 버튼, 경로는 `/participants/:id/programs/:caseId/report`.

### 실화면 결과 (합성 미리보기)

| # | 확인 | 결과 |
|---|---|---|
| 1 | 기록 목록에서 리포트 입구 노출 | 통과 |
| 2 | 리포트 열림, 제목 `전체 상담 리포트` | 통과 |
| 3 | 회차 목록(요약 있는 회차·없는 회차) | 통과 |
| 4 | 요약 근거 없는 회차의 대체 문구 | 통과 |
| 5 | 근거 있는 네 구획 렌더 | 통과 |
| 6 | 근거 없는 `위험 신호` 구획 카드 미생성 | 통과(제목 목록에 없음) |
| 7 | 빠진 구획 안내가 "자료 없음"으로 읽히게 표시 | 통과 |
| 8 | 첫 인테이크 목표를 현재 목표로 대체하지 않음 | 통과 |
| 9 | 근거별 회차 링크 7개 | 통과 |
| 10 | 다음에 확인할 것(인테이크 후속 질문) | 통과 |
...
## 32. 실서버 결함 교정 3: 인테이크 일정과 목표 (2026-09-10)

**증상(Main 실런타임):** 인테이크 전 케이스로 일정을 등록하면 화면은 인테이크로 등록한다고 하면서도
`이번 상담의 목표` 칸을 그대로 보여 줬고 `POST /schedules` 가 400 `invalid_request` 로 떨어졌다.

**서버 규칙:** 인테이크 일정은 세션 목표를 가질 수 없고(`createIntakeCounselingSchedule`),
세부 목표(`caseGoals`)는 인테이크 경로에서만 만든다(D62).

**고친 것**

1. `SchedulesApi.create` 가 목표를 상담 유형에 따라 다른 자리로 보낸다. 인테이크는 `caseGoals`
   문자열 배열만, 기본 상담은 `sessionGoals` 만 싣는다. 두 키를 함께 보내는 경로가 없다.
2. 등록 화면이 인테이크일 때 칸 이름을 `이 사업의 세부 목표` 로 바꾸고, 적은 목표가 세부 목표로
   저장된다는 사실을 안내에 적는다. 칸을 숨기지 않은 이유는 D62 가 인테이크에서 세부 목표를
   세우는 자리로 정하기 때문이다.
3. 403 은 이 화면에서 `이 사업에 담당으로 배정된 실무자만 일정을 등록할 수 있습니다. 담당 배정을
   먼저 받아 주세요.` 로 답한다. 입구는 그대로 둔다. 후보 목록 응답에 담당 여부가 없어 화면이
   숨기려면 새 필드가 필요하고, 그것은 추측 API 다. 필드가 생기면 그때 목록에서 가른다.
4. 하네스가 실제 서버와 같은 규칙을 강제한다: 인테이크+세션 목표 400, 기본 상담+세부 목표 400.
   인테이크 전 후보(`intakeAt: null`)도 하나 넣어 이 경로가 검수에서 실제로 지나가게 했다.

**검증**

| # | 확인 | 결과 |
|---|---|---|
| 1 | 단위: 인테이크는 `caseGoals` 만, `sessionGoals` 키 없음 | 통과 |
| 2 | 단위: 기본 상담은 `sessionGoals` 만, `caseGoals` 키 없음 | 통과 |
| 3 | 단위: 빈 줄은 걸러 빈 목록 | 통과 |
| 4 | 화면: 인테이크 칸 이름과 안내 | 통과 |
| 5 | 화면: 인테이크 일정 등록 성공(계획 화면 이동) | 통과 |
| 6 | 화면: 기본 상담 일정 등록 성공 | 통과 |
| 7 | 하네스가 옛 조합(인테이크+세션 목표) 400 거절 | 통과 |
| 8 | 하네스가 기본 상담+세부 목표 400 거절 | 통과 |

client 72 tests, typecheck, build, tokens·align·hierarchy 통과.

## 33. 실서버 결함 교정 4: 공개 표면 두 건 (2026-09-10)

### 1. 공개 가입을 끈 설치의 당사자 초대

**증상:** 스위치가 꺼져 있으면 `POST /invites/participant` 가 설계대로 404 인데(CCC-112) 화면은
`서버에 연결할 수 없습니다. 잠시 뒤 다시 시도해 주세요.` 라고 해서 될 리 없는 재시도를 권했다.

**고친 것**: `GET /capabilities` 의 `features.public_signup` 을 읽어 사이드바에서 `당사자 초대`
자리를 감춘다. 주소로 직접 들어오면 화면이 `이 설치는 요청 링크를 쓰지 않습니다` 와 당사자 등록
경로를 안내한다. 발급 호출의 404 는 전송 오류가 아니라 `public_signup_disabled` 로 옮겨
같은 사실을 말한다.

### 2. 이미 쓴 링크를 다시 열었을 때

**증상:** 방금 접수한 것처럼 `등록 완료 / 접수했습니다` 가 그대로 보였다.

**원인:** 같은 주소에 같은 조각으로 다시 들어가면 브라우저가 문서를 새로 읽지 않는다. 화면의
제출 완료 상태가 남아 있었고, 조각 훅이 같은 토큰이라 다시 돌지도 않았다.

**고친 것**: 조각 훅이 링크를 읽을 때마다 오르는 `nonce` 를 함께 돌려주고, 그 값으로 조회를 다시
돌리며 제출 완료 상태를 지운다. 이미 쓴 링크는 서버가 준 문구(`이 링크는 이미 사용되었습니다.
담당 실무자에게 문의해 주세요.`)를 그대로 쓰고 담당 실무자 이름을 한 줄 덧붙인다. 실무자 초대
수락 화면도 같은 방식으로 상태를 지운다.

### 검증

| # | 확인 | 결과 |
|---|---|---|
| 1 | 스위치 끔: 사이드바에서 당사자 초대 사라짐 | 통과 |
| 2 | 스위치 끔: 주소로 들어와도 설명, 재시도 문구 없음 | 통과 |
| 3 | 단위: 기능이 꺼지면 자리 목록에서 빠짐 | 통과 |
| 4 | 스위치 켬: 링크 발급과 제출 성공 | 통과 |
| 5 | 같은 주소 재진입에서 이미 사용 상태 표시 | 통과 |
| 6 | 서버 문구와 담당 실무자 이름 표시 | 통과 |
| 7 | 접수 완료 문구가 남지 않음 | 통과 |
| 8 | 새로 연 문서에서도 이미 사용 상태 | 통과 |
