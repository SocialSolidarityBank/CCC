...
## 6.1 실패의 두 종류 (2026-09-11 판정)

`apply`의 무결성 약속은 단계에 따라 뜻이 다르다. 뭉뚱그리면 지킬 수 없는 약속이 된다.

- 첫 마이그레이션 이전은 **쓰기 0건**이다. 묶음, manifest, tuple, artifact hash, Edge component
  집합, component 종류 거부, 신뢰 역할, 백업 면제 판단이 모두 lock 이전에 끝난다. 이 구간의
  어떤 실패도 데이터베이스를 건드리지 않으며, 그 사실을 쓰기 수로 증명한다.
- 첫 마이그레이션 이후는 **승격 0건**이다. installed receipt, release history, known-good 기록을
  남기지 않고, journal을 재개 가능한 미완료 상태로 명시해 성공으로 오독될 수 없게 한다. 개별
  journaled 마이그레이션은 적용된 채 남을 수 있다. S11은 전진 전용이고 검증된 역방향
  마이그레이션이 없으므로, 원자성을 흉내 내려고 down 마이그레이션이나 교차 트랜잭션을 지어내지
  않는다. health 실패 뒤 installed로 표기하는 것은 금지한다.
- 두 구간의 실패는 서로 다른 고정 code로 구분한다. 재개 실행은 journal을 읽어 이어가야 하며,
  미완료 상태를 빈 프로젝트로 오인하지 않는다. 따라서 그 재개에는 첫 설치 백업 면제가 적용되지
  않는다.
