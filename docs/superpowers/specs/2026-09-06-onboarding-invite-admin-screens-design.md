...
| 6 | 결과 카드: 링크(16, 당사자 초대 규칙과 같음)+`복사`, `이메일 문안 복사` | ③+버튼 | `WireCard`+`WireField`+`WireButton` secondary | 발급 후 |
...
| 5 | 링크 주소 | 16 값 | `WireField` (당사자에게 보여 주는 값 16, 설명 14 규칙 유지, `.participant-invite-stack`) |
| 6 | QR | 이미지 | 현행 SVG |
| 7 | `복사` / `문안 복사` | 버튼 | `WireButton` secondary ×2. 카카오톡·문자 버튼은 **두지 않는다** |
