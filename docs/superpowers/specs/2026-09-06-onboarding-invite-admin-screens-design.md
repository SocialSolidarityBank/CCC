...
| 6 | 결과 카드: 링크(16, 당사자 초대 규칙과 같음), 아래 버튼 줄 `링크 복사` `공유하기`, `이메일 문안 복사` | ③+버튼 | `WireCard`+`WireFormField`+`.wizard-actions` 줄 안 `WireButton` secondary(공유는 `icon` 슬롯) | 발급 후. `공유하기`는 공유 시트가 있는 브라우저에서만 |
...
| 5 | 링크 주소 | 16 값 | `WireFormField`(당사자에게 보여 주는 값 16, `.participant-invite-stack`) |
| 6 | 버튼 줄 `링크 복사` `공유하기` | 버튼 | `.wizard-actions`(왼쪽부터 차는 버튼 줄) 안 `WireButton` secondary ×2. 공유는 `icon={<NavIcon name="share" />}` + 글자 `공유하기`. 카카오톡·문자 버튼은 **두지 않는다** |
| 7 | QR | 이미지 | 현행 SVG |
| 8 | `이메일 문안 복사` | 버튼 | `WireButton` secondary |

`공유하기`는 OS 공유 시트(`navigator.share`)를 띄우고 실무자가 카카오톡·문자·메일 중 고른다. CCC는 어느 채널로 갔는지 모른다(D86 ④ "링크는 전달 채널을 모른다" 유지). 공유 시트를 못 띄우는 브라우저(데스크톱 Firefox 등)에서는 버튼을 렌더하지 않는다. 자리 예약 없이 `링크 복사`만 남는다. 마운트 뒤 기능 감지로 켜서 서버 렌더와 어긋나지 않게 한다. 공유 문안은 `이메일 문안 복사`와 같은 문안이고 **당사자 이름을 넣지 않는다**: 기관 이름, 담당 실무자 이름, "7일 안에 한 번만 열 수 있습니다", 링크. 실무자 개인 메신저 계정 사용 여부는 기관 운영 규칙(배포기관 서식)이고 CCC는 강제하지 않는다(2026-09-06 Q 확정, ADR 없음).

**2026-09-06 Q 2차 개정**: 1차 구현(입력칸 오른쪽 `.header-icon-button` 32 원, #283)은 프리뷰에서 눈에 띄지 않아 폐기했다. 공유는 `링크 복사`와 같은 링크에 대한 두 조작이라 같은 버튼 줄에 같은 옷(secondary 알약)으로 서고, 아이콘+글자로 무엇인지 읽힌다. `.wire-field-with-action`은 입력 값 자체를 만드는 조작(세부 목표 추가)에만 남긴다.
...
5. `ShellIconName`에 `'share'` 추가(`shell-icons.tsx`, 16px 라인 아이콘, `currentColor`). D58 ⑦ 아이콘 공용 SVG 시스템이라 문자 글리프나 별도 아이콘 파일을 두지 않는다
6. `WireButton`에 `icon?: ReactNode` 슬롯 추가(글자 앞 16px, 꺽쇠와 같은 `--nudge-hangul` 보정). 아이콘만 있는 버튼은 `WireRepeatActions` 어휘라 이 슬롯은 글자와 함께만 쓴다. 새 클래스 0
...
- 발급 화면 1280에서 `링크 복사`·`공유하기`가 같은 32 높이, 공유 아이콘과 글자가 세로 중앙 공유, 390에서 두 버튼이 한 줄에 겹치지 않음(`pnpm design:align` `height`, `y`, `no-overlap-x-each`)
