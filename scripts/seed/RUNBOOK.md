...
- **날짜는 기준일에서 상대 계산된다** — 기본 기준일은 시드를 만드는 날(KST)이라, 만들 때마다
  '다가오는 상담'이 항상 채워진다. 어느 기준일로 만들어졌는지는 `manifest.json` 의
  `anchorDate` 에 적히고, 과거 산출물을 그대로 재현하려면 그 값을 넣는다:
  `SEED_ANCHOR_DATE=2026-07-19 pnpm exec vitest run --config scripts/seed/vitest.config.ts`.
- `yellow` **이미 넣은 DB 는 재생성으로 고쳐지지 않는다** — 기존 예정 일정을 앞으로 당기는
  일은 `pnpm seed:reschedule`(`scripts/seed/reschedule-upcoming.mjs`)이 맡는다. UPDATE SQL 만
  만들어 내고 적용은 사람이 한다.
...
- 라이브 스모크: 브리핑/일정 화면에서 담당자 계정으로 실명 렌더 + 예정 일정(ai00 담당 4건)이
  보이는지 확인한다. 예정 일정은 **기준일 +3~+6 일**이라 시드를 만든 날 기준으로 항상
  '다가오는 상담'(향후 7일) 안에 들어온다 — 날짜가 하드코딩이던 시절의 "그 주가 지나면
  비어 보인다" 문제는 없어졌다.
