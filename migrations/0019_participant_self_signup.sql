-- ============================================================================
-- 마이그레이션 0019 - 참여자 자기 가입(self signup)을 위한 동의 기록자 + 토큰
-- 이중 소비 가드 (D39 · ADR-0016 · CCC-28)
--
-- 배경: 참여자는 users 디렉터리에 들지 않고 고유 토큰 링크로 가입한다(ADR-0016
--   결정 4·6). 가입 제출은 한 트랜잭션 안에서 참여자+케이스+담당 배정+동의 기록
...
--      참여자가 아직 디렉터리에 없으므로 기록자를 'self' 로 둔다(ADR-0016 결정 6,
...
--      참여자를 만들고 진 쪽의 소비 UPDATE 가 0 행을 맞춰 고아 참여자를 남긴다.
...
-- 1) 동의 기록자 가드: 'self'(자기 가입) 또는 같은 조직의 활성 담당 사용자.
...
BEGIN
  ...
  -- 'self' 는 자기 가입(참여자 본인)의 기록자 표식. 그 외는 기존대로 활성 사용자여야 한다.
  SELECT RAISE(ABORT, 'participant_schema_violation')
