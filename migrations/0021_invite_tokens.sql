-- ============================================================================
-- 마이그레이션 0018 — 초대 토큰 테이블 (D39 · ADR-0016 · CCC-29)
--
-- 1차 MVP 가입 흐름의 기반. 두 종류의 초대를 한 테이블이 담는다:
--   * participant — 참여자 가입 링크(사업+발급 상담사 묶음, ADR-0016 결정 5).
--     가입 완료 시 참여자+케이스가 만들어지고 발급 상담사가 담당자가 된다.
--     같은 토큰이 가입 후에는 참여자 자기 확인 페이지의 열람 자격이 된다.
--   * counselor — 상담사 초대 링크(관리자 발급, 가입 시 users 등재. CCC-33).
--
-- 토큰이 곧 자격(로그인 없음)이므로:
...
-- users.id 를 가리킨다(자기 확인 페이지가 토큰 → 참여자를 푸는 연결 고리).
--
