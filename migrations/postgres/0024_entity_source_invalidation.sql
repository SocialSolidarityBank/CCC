-- Logical pair: SQLite 0068_entity_source_invalidation.sql.
-- Replace the two order-sensitive source triggers with one atomic transition.
DROP TRIGGER IF EXISTS agent_text_source_superseded ON counseling_memory_cases;
DROP TRIGGER IF EXISTS agent_text_source_successor ON counseling_memory_cases;
DROP TRIGGER IF EXISTS agent_entity_map_source_superseded ON support_cases;
DROP TRIGGER IF EXISTS organization_settings_time_zone_invalidation ON organization_settings;
DROP FUNCTION IF EXISTS agent_text_source_superseded_fn();
DROP FUNCTION IF EXISTS agent_text_source_successor_fn();
DROP FUNCTION IF EXISTS agent_entity_map_source_superseded_fn();

CREATE FUNCTION agent_text_source_invalidation_fn() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  ...
  WHERE org_id=NEW.org_id AND support_case_id=NEW.support_case_id
    ...
    AND (entity_source_binding IS NULL OR COALESCE((entity_source_binding::jsonb ->> 'generation')::bigint,-1) <> NEW.generation)
  ...
  INSERT INTO agent_jobs(id,org_id,support_case_id,session_id,source_text_work_item_id,kind,state,
   ...
   SELECT replace(gen_random_uuid()::text,'-',''),j.org_id,j.support_case_id,j.session_id,j.source_text_work_item_id,
     'text','pending',to_char(statement_timestamp() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
     j.required_consent,j.consent_revision,j.consent_receipt_json,0,NEW.generation,
...
FOR EACH ROW WHEN (NEW.generation <> OLD.generation) EXECUTE FUNCTION agent_text_source_invalidation_fn();

CREATE FUNCTION organization_settings_time_zone_invalidation_fn() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  ...
  UPDATE counseling_memory_cases SET generation=generation+1,status='updating',request_json=NULL,
    egress=NULL,lease_token=NULL,lease_until=NULL,
    not_before=to_char(statement_timestamp() AT TIME ZONE 'UTC' + interval '5 seconds','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')
  WHERE org_id=NEW.org_id;
  RETURN NEW;
END;
$$;
CREATE TRIGGER organization_settings_time_zone_invalidation AFTER UPDATE OF time_zone ON organization_settings
FOR EACH ROW WHEN (NEW.time_zone IS DISTINCT FROM OLD.time_zone)
EXECUTE FUNCTION organization_settings_time_zone_invalidation_fn();

CREATE FUNCTION agent_entity_map_invalidation_fn() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  ...
  WHERE org_id=NEW.org_id AND id=NEW.id AND NOT (
    (entity_map_lease_family='generic' AND entity_map_lease_job_id IS NOT NULL AND EXISTS(
      SELECT 1 FROM agent_jobs j WHERE j.id=entity_map_lease_job_id AND j.org_id=NEW.org_id
        ...
        AND COALESCE((j.entity_source_binding::jsonb ->> 'mapRevision')::bigint,-1)=NEW.entity_map_revision))
    OR (entity_map_lease_family='memory' AND entity_map_lease_job_id IS NOT NULL AND EXISTS(
      SELECT 1 FROM counseling_memory_materials m WHERE m.id=entity_map_lease_job_id AND m.org_id=NEW.org_id
        ...
        AND COALESCE((m.entity_source_binding::jsonb ->> 'mapRevision')::bigint,-1)=NEW.entity_map_revision))
  );
...
FOR EACH ROW WHEN (NEW.entity_map_revision <> OLD.entity_map_revision)
EXECUTE FUNCTION agent_entity_map_invalidation_fn();
