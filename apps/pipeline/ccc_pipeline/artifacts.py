...
Claude 대조·요약·플래그 제안은 2단계-c 범위(D16·D18 대기)라 지금은 빈 값/자리표시로
...
# aiSummary는 서버가 비어 있으면 거부한다. 2단계-c(Claude 대조 프롬프트) 연결 전까지
# 미생성임이 화면에서 명확히 보이는 고정 문구를 보낸다.
AI_SUMMARY_PLACEHOLDER = "(AI 요약 미생성 — Claude 대조·요약은 2단계-c 연결 후 제공)"
...
def build_artifacts(transcript: str, emotion_scores: dict[str, float]) -> dict[str, Any]:
    ...
    return {
        ...
        "flagProposals": [],  # AI 플래그 제안은 전사 인용이 필수(D9) — Claude 연결(2단계-c) 후
