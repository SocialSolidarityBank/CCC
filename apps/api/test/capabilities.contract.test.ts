...
import { seedTestProgramWithRuntimeModes, setupD1, testActors } from './support/d1';
...
describe('GET /capabilities', () => {
  ...
  async function envWithManifest(mode: DeploymentMode) {
    await t.reset();
    await seedTestProgramWithRuntimeModes(t.db, testActors.admin.orgId, testActors.admin.userId, {
      deploymentMode: mode, sttMode: 'local', llmMode: 'off',
    });
