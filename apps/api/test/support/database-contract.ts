...
export function defineDatabaseContract(
  name: string,
  openDatabase: () => Promise<Database>,
  options: { dialect?: 'sqlite' | 'postgres' } = {},
): void {
    ...
    if (options.dialect === 'postgres') {
      for (const sql of [
        `CREATE TABLE database_port_fixture (
          ...
          value TEXT, payload BYTEA, marker TEXT NOT NULL UNIQUE
        )`,
        'CREATE TABLE database_port_primary (id BIGINT PRIMARY KEY, value TEXT)',
        'CREATE TABLE database_port_trigger (id BIGINT PRIMARY KEY, value TEXT)',
        'CREATE TABLE database_port_allowed_trigger (id BIGINT PRIMARY KEY)',
        `CREATE FUNCTION database_port_reject() RETURNS trigger LANGUAGE plpgsql AS '
          BEGIN RAISE EXCEPTION USING MESSAGE = TG_ARGV[0]; END;
        '`,
        `CREATE TRIGGER database_port_unknown_trigger BEFORE INSERT ON database_port_trigger
          FOR EACH ROW EXECUTE FUNCTION database_port_reject('unknown_trigger_code')`,
        `CREATE TRIGGER database_port_allowed_trigger_check BEFORE INSERT ON database_port_allowed_trigger
          FOR EACH ROW EXECUTE FUNCTION database_port_reject('invite_token_already_used')`,
  ...
  it('normalizes binary values to owned Uint8Array copies for input and output', async () => {
    ...
    const backing = new Uint8Array([0xaa, 0x00, 0xff, 0xbb]);
    const view = new Uint8Array(backing.buffer, 1, 2);
    const offsetBound = db.prepare(
      'INSERT INTO database_port_fixture (value, payload, marker) VALUES (?, ?, ?)',
    ).bind('offset-view', view, 'offset-copy');
    backing.fill(0x7f);
    await offsetBound.run();
    if (options.dialect !== 'postgres') {
      // Preserve the native D1/SQLite ArrayBuffer compatibility regression.
    ...
    const offsetRow = await db.prepare(
      'SELECT payload FROM database_port_fixture WHERE marker = ?',
    ).bind('offset-copy').first<{ payload: Uint8Array }>();
    expect(Array.from(offsetRow?.payload ?? [])).toEqual([0x00, 0xff]);
  });
