...
async function postgresCatalog(db: Database): Promise<Catalog> {
  ...
  const security = {
    policies: await pgRows(db, `SELECT n.nspname AS schema,c.relname AS table_name,p.polname AS name,p.polcmd,p.polpermissive,ARRAY(SELECT CASE WHEN role_id=0 THEN 'PUBLIC' ELSE pg_get_userbyid(role_id)::text END FROM unnest(p.polroles) role_id ORDER BY 1) AS roles,pg_get_expr(p.polqual,p.polrelid) AS using_expression,pg_get_expr(p.polwithcheck,p.polrelid) AS check_expression FROM pg_policy p JOIN pg_class c ON c.oid=p.polrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname IN ('public','private') ORDER BY n.nspname,c.relname,p.polname`),
    relations: await pgRows(db, `SELECT n.nspname AS schema,c.relname AS name,c.relkind,c.relrowsecurity,c.relforcerowsecurity,pg_get_userbyid(c.relowner)::text AS owner,ARRAY(SELECT a::text FROM unnest(COALESCE(c.relacl,acldefault(CASE WHEN c.relkind='S' THEN 's'::"char" ELSE 'r'::"char" END,c.relowner))) a ORDER BY 1) AS grants,ARRAY(SELECT option FROM unnest(c.reloptions) option ORDER BY 1) AS options FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname IN ('public','private') AND c.relkind IN ('r','p','v','m','S') ORDER BY n.nspname,c.relname`),
    columns: await pgRows(db, `SELECT n.nspname AS schema,c.relname AS table_name,a.attname AS name,ARRAY(SELECT grant_entry::text FROM unnest(a.attacl) grant_entry ORDER BY 1) AS grants FROM pg_attribute a JOIN pg_class c ON c.oid=a.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname IN ('public','private') AND a.attnum>0 AND NOT a.attisdropped AND a.attacl IS NOT NULL ORDER BY n.nspname,c.relname,a.attnum`),
    functions: await pgRows(db, `SELECT n.nspname AS schema,p.proname AS name,pg_get_function_identity_arguments(p.oid) AS arguments,pg_get_functiondef(p.oid) AS definition,pg_get_userbyid(p.proowner)::text AS owner,p.prosecdef,ARRAY(SELECT a::text FROM unnest(COALESCE(p.proacl,acldefault('f',p.proowner))) a ORDER BY 1) AS grants,ARRAY(SELECT setting FROM unnest(p.proconfig) setting ORDER BY 1) AS settings FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname IN ('public','private') AND p.prokind IN ('f','p') ORDER BY n.nspname,p.proname,pg_get_function_identity_arguments(p.oid)`),
    schemas: await pgRows(db, `SELECT n.nspname AS name,pg_get_userbyid(n.nspowner)::text AS owner,ARRAY(SELECT a::text FROM unnest(COALESCE(n.nspacl,acldefault('n',n.nspowner))) a ORDER BY 1) AS grants FROM pg_namespace n WHERE n.nspname IN ('public','private') ORDER BY n.nspname`),
    roles: await pgRows(db, `SELECT rolname,rolsuper,rolinherit,rolcreaterole,rolcreatedb,rolcanlogin,rolreplication,rolbypassrls,rolconnlimit,ARRAY(SELECT setting FROM unnest(rolconfig) setting ORDER BY 1) AS settings FROM pg_roles WHERE rolname IN ('ccc_schema_owner','ccc_api','anon','authenticated') ORDER BY rolname`),
    memberships: await pgRows(db, `SELECT pg_get_userbyid(m.roleid)::text AS role,pg_get_userbyid(m.member)::text AS member,m.admin_option,m.inherit_option,m.set_option FROM pg_auth_members m WHERE pg_get_userbyid(m.roleid) IN ('ccc_schema_owner','ccc_api','anon','authenticated') OR pg_get_userbyid(m.member) IN ('ccc_schema_owner','ccc_api','anon','authenticated') ORDER BY 1,2`),
    defaults: await pgRows(db, `SELECT pg_get_userbyid(d.defaclrole)::text AS owner,n.nspname AS schema,d.defaclobjtype,ARRAY(SELECT a::text FROM unnest(d.defaclacl) a ORDER BY 1) AS grants FROM pg_default_acl d LEFT JOIN pg_namespace n ON n.oid=d.defaclnamespace WHERE n.nspname IN ('public','private') OR d.defaclnamespace=0 ORDER BY 1,2,3`),
  };
  ...
  }), indexes: indexes.filter((entry) => !entry.constraint_owned).map((entry) => ({ name: String(entry.name), table: String(entry.table_name), unique: entry.indisunique ? 1 : 0, columns: stringArray(entry.columns).map((column) => column.replace(/"/g, '').replace(/\s+(ASC|DESC)(\s+NULLS\s+(FIRST|LAST))?$/i, '')), predicate: entry.predicate === null ? null : String(entry.predicate), definition: String(entry.definition) })), triggers: triggers.map((entry) => ({ name: String(entry.name), table: String(entry.table_name), definition: String(entry.definition), function: String(entry.function) })), views: views.map((entry) => ({ name: String(entry.name), definition: String(entry.definition) })), physical: [{ tables, columns, constraints, indexes, triggers, views, functions, sequences, security }] });
}
...
export const physicalRules = {
  ...
  security: 'PostgreSQL RLS enable/force flags, policy roles and expressions, relation/function/schema/column grants, ownership, view options, role attributes/memberships and default grants are fingerprinted without passwords or object OIDs. SQLite has no server-role equivalent; the guard organization column remains logical schema in both engines.',
} as const;
