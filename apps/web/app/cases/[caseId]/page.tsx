...
export default async function LegacyCasePage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  return <main className="page-content narrow"><header className="page-header"><div><h1>상담 준비</h1><p>이전 주소의 참여자 정보를 확인할 수 없습니다.</p></div></header><p className="status risk" role="alert">참여자 ID로 확인할 수 없는 이전 주소입니다.</p><Link className="detail-link" href="/"><span>참여자 목록으로 돌아가기</span></Link></main>;
}
