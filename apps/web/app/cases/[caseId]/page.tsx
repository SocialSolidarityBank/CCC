...
export default async function LegacyCasePage({ params }: { params: Promise<{ caseId: string }> }) {
  ...
  return <main className="page-content"><header className="page-header"><div><h1>상담 준비</h1><p>이전 주소의 당사자 정보를 확인할 수 없습니다.</p></div></header><p className="wire-badge" data-tone="risk" role="alert">당사자 ID로 확인할 수 없는 이전 주소입니다.</p><div><WireButton variant="secondary" href="/">당사자 목록으로 돌아가기</WireButton></div></main>;
}
