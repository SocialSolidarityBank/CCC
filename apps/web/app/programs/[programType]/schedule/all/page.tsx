...
// 구 '전체 일정' 경로. 일정 화면 통합(D75 · ADR-0039)으로 화면은 `/schedule?range=month`
// 하나가 됐고, 이 경로는 북마크·과거 링크를 살리는 리다이렉트만 남았다. month 쿼리는
// 그대로 넘긴다 — 검증은 통합 화면이 한다(형식이 어긋나면 이번 달로 떨어진다).
export default async function ProgramScheduleAllPage({
  ...
  redirect(`/programs/${encodeURIComponent(programType)}/schedule?range=month${month}`);
}
