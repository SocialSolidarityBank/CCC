...
describe('사업 전환기 — 사업이 2개 이상일 때 (2026-07-31)', () => {
  it('닫혀 있을 때는 현재 사업만 보이고 목록은 없다', () => {
    ...
    expect(container.querySelector('.sidebar .program-switcher-menu')).toBeNull();
  });

  it('누르면 사업 전부가 뜨고 현재 사업에 선택 표시가 붙는다', () => {
    const { container } = render(<AppSidebar activePath="/programs/financial_support_v1/schedule" />);
    fireEvent.click(container.querySelector('.sidebar .program-switcher-trigger') as HTMLElement);

    const options = Array.from(container.querySelectorAll('.sidebar .program-switcher-option'));
    ...
    expect(container.querySelectorAll('.sidebar .program-switcher-option[aria-current="true"]')).toHaveLength(1);
  });
  ...
  it('listbox 역할을 참칭하지 않는다 — 화살표 이동을 구현하지 않았으므로', () => {
    ...
    expect(container.querySelector('.sidebar .program-switcher-trigger')?.getAttribute('aria-haspopup')).toBeNull();
  });

  it('사업을 바꾸면 그 사업의 일정으로 간다 — 워크스페이스를 옮기는 것이므로', () => {
    const { container } = render(<AppSidebar activePath="/programs/financial_support_v1/schedule" />);
    fireEvent.click(container.querySelector('.sidebar .program-switcher-trigger') as HTMLElement);
    const hrefs = Array.from(container.querySelectorAll('.sidebar .program-switcher-option'))
  ...
  it('Escape 로 닫힌다 — 스크림이 없으므로 키보드 탈출구가 있어야 한다', () => {
    const { container } = render(<AppSidebar activePath="/programs/financial_support_v1/schedule" />);
    fireEvent.click(container.querySelector('.sidebar .program-switcher-trigger') as HTMLElement);
    expect(container.querySelector('.sidebar .program-switcher-menu')).not.toBeNull();
