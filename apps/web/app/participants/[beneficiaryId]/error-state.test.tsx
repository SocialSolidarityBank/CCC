...
describe('당사자 정보 차단 안내 (CCC-23)', () => {
  it('복귀 링크가 당사자 목록을 가리킨다', () => {
    const { container } = render(<ErrorState kind="access_or_not_found" />);
    const link = within(container).getByRole('link', { name: '당사자 목록으로 돌아가기' });
    expect(link.getAttribute('href')).toBe('/participants');
  });

  it('안내 문구가 당사자의 존재 여부를 드러내지 않는다 (D36 전제 게이트)', () => {
    // "없는 당사자"와 "권한 없음"을 구분하지 않는다 — 존재 자체가 정보다.
    const { container } = render(<ErrorState kind="access_or_not_found" />);
