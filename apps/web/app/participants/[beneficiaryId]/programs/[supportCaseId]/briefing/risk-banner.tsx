...
export function RiskBanner({ flags, recordsHref }: { flags: RiskBannerFlag[]; recordsHref: string }) {
      ...
      <ul className="risk-banner-list">
        {confirmedFlags.map((flag) => (
          <li key={flag.id}>
            ...
            {flag.quote !== null && flag.sessionId !== null && (
              <WireSourceQuotes
                quotes={[flag.quote]}
                sourceHref={`${recordsHref}#record-${flag.sessionId}`}
              />
            )}
          </li>
        ))}
