...
const readyProgramOptions = [{
  id: 'program-ready',
  displayName: '희망키움 2026',
...
describe('RegisterForm (#37 당사자 등록 폼)', () => {
  it('requires an explicit admitted program selection and excludes locked programs', () => {
    const { container } = render(
      <RegisterForm
        currentUser={currentUser}
        action={noop}
        programOptions={[
          { id: 'program-ready', displayName: '희망키움 2026', programType: 'financial_support_v1', admissionState: 'ready' },
          { id: 'program-locked', displayName: null, programType: 'financial_support_v1', admissionState: 'confirmation_required' },
        ]}
      />,
    );
    const select = container.querySelector('select[name="programId"]') as HTMLSelectElement;
    expect(select.required).toBe(true);
    expect([...select.options].map((option) => option.value)).toEqual(['', 'program-ready']);
    fireEvent.change(select, { target: { value: 'program-ready' } });
    expect(new FormData(container.querySelector('form') as HTMLFormElement).get('programId')).toBe('program-ready');
  });
  it('disables creation when no program is available', () => {
    const { container } = render(<RegisterForm currentUser={currentUser} action={noop} programOptions={[]} />);
    expect(container.querySelector('select[name="programId"] option')?.getAttribute('value')).toBe('');
  ...
  it('orders 성별 above 생년월일', () => {
    const { container } = render(<RegisterForm currentUser={currentUser} action={noop} programOptions={readyProgramOptions} />);
  ...
  it('shows an invalid email beside the field and preserves the other entered values', () => {
    const action = vi.fn();
    const { container } = render(<RegisterForm currentUser={currentUser} action={action} programOptions={readyProgramOptions} />);
    const form = container.querySelector('form') as HTMLFormElement;
  ...
  it('falls back to the email when the registrant has no display name', () => {
    const { container } = render(
      <RegisterForm currentUser={{ name: null, email: 'noname@example.test' }} action={noop} programOptions={readyProgramOptions} />,
    );
