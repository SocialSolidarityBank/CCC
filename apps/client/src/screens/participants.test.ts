...
import { requiredIntakeQuestionKeys, type IntakeCreateRequest, type IntakeUpdateRequest, type IntakeSavedRecord } from '@ccc/contracts/intake';
...
async function harness(options: {
  ...
  saved?: boolean; legacy?: boolean; financialSupportEnabled?: boolean; writeFailure?: 'conflict' | 'unavailable'; routeBeneficiaryId?: string;
} = {}) {
  ...
  let current: IntakeSavedRecord | null = options.legacy ? { sessionId: 'intake-1', heldAt, channel: 'phone', revision: 1,
    history: [], schemaVersion: 1, questionnaire: null, legacyDetailsJson: legacyJson }
    : options.saved ? { sessionId: 'intake-1', heldAt, channel: 'phone', revision: 1, history: [],
      schemaVersion: 2, questionnaire, legacyDetailsJson: null } : null;
  ...
  const transport = { request: async (path: string, method = 'GET', body?: IntakeCreateRequest | IntakeUpdateRequest) => {
    ...
    if (path === contextPath && method === 'GET') {
      ...
      return { beneficiaryId, supportCaseId: CASE_ID, participant: { name: null, phone: null, email: null },
        sessionSequence: 1, hasIntake: current !== null, writeSchemaVersion: 2, moduleSnapshot,
    ...
    if (path === contextPath && body) {
      ...
      if (writeFailure) {
        ...
        if (failure === 'conflict' && current) current = { ...current, revision: current.revision + 1 };
        throw new BusinessError(failure, failure === 'conflict' ? 409 : 503);
      }
      const history = current ? [{ revision: current.revision, schemaVersion: current.schemaVersion, heldAt: current.heldAt,
        ...
        detailsJson: current.schemaVersion === 1 ? current.legacyDetailsJson : JSON.stringify(current.questionnaire) }, ...current.history] : [];
      current = { schemaVersion: 2, sessionId: 'intake-1', heldAt: body.heldAt, channel: body.channel,
        questionnaire: body.questionnaire, legacyDetailsJson: null, revision: (current?.revision ?? 0) + 1, history };
      return { schemaVersion: 2, revision: current.revision, replayed: false,
...
describe('versioned intake journey', () => {
  it('registers once, follows both returned IDs, reads one context and submits applicable v2 answers', async () => {
    ...
    expect(h.writes[0]).toMatchObject({ schemaVersion: 2, channel: 'video', questionnaire: { moduleSnapshot: intakeModule,
      linkedOrgs: { response: 'not_applicable' }, additionalItems: { response: 'declined' }, debts: null } });
  ...
  it('requires explicit legacy conversion, preserving channel and original revision without reclassification', async () => {
    ...
    expect(h.writes[0]).toMatchObject({ schemaVersion: 2, expectedRevision: 1, heldAt, channel: 'phone', conversion: { confirmed: true, sourceRevision: 1 } });
