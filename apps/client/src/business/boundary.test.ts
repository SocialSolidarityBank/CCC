...
describe('첫 로그인 신원 연결', () => {
  it('디렉터리 거절을 한 번만 연결로 복구하고 원래 요청을 다시 부른다', async () => {
    const requests: Request[] = [];
    let linked = false;
    const transport = new BusinessTransport(await installation(), () => 'synthetic-token', async (input, init) => {
      const request = new Request(input, init);
      ...
      if (pathname.endsWith('/identity/link')) {
        linked = true;
        return json({ linked: true });
      }
      if (!linked) return json({ error: 'forbidden' }, 403);
      return pathname.endsWith('/capabilities')
        ? json(capabilities(), 200, { 'X-CCC-Installation-Id': installationId }) : json({ roles: ['worker'] });
    });
    await expect(transport.initialize()).resolves.toMatchObject({ mode: 'community-cloud' });
    await transport.request('/me');
    const link = requests.filter((request) => request.url.endsWith('/identity/link'));
    expect(requests.map((request) => request.url.slice(apiBase.length))).toEqual([
      '/capabilities', '/identity/link', '/capabilities', '/me',
    ]);
    expect(await link[0]?.clone().json()).toEqual({});
    expect(link[0]?.headers.get('authorization')).toBe('Bearer synthetic-token');
    expect(link[0]?.credentials).toBe('omit');
    transport.dispose();
  });

  it('연결이 거절되면 원래 거절을 그대로 올리고 다시 연결하지 않는다', async () => {
    const requests: string[] = [];
    const transport = new BusinessTransport(await installation(), () => 'synthetic-token', async (input) => {
      const path = String(input).slice(apiBase.length);
      requests.push(path);
      return path === '/identity/link'
        ? json({ error: 'identity_not_invited' }, 403) : json({ error: 'forbidden' }, 403);
    });
    await expect(transport.initialize()).rejects.toMatchObject({ status: 403, code: 'forbidden' });
    await expect(transport.initialize()).rejects.toMatchObject({ status: 403, code: 'forbidden' });
    expect(requests.filter((path) => path === '/identity/link').length).toBe(1);
    transport.dispose();
  });
});
