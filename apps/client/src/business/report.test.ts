...
const report = {
  schemaVersion: 1, supportCaseId: 'case-1', beneficiaryId: 'swallow-003', programId: 'program-1', programName: '사업', status: 'active',
  sessions: [{ sessionId: 'session-1', sessionNumber: 1, heldAt: evidence.heldAt, kind: 'intake', channel: 'phone',
    intakeSchemaVersion: 2, intakeRevision: 4, summary: evidence }],
  firstIntakeGoal: evidence, sections: { situationChanges: { entries: [evidence] } },
};

describe('report intake source version preservation', () => {
  it('retains provenance on sessions and every evidence projection without changing the report version', () => {
    const decoded = decodeReport(report);
    expect(decoded.schemaVersion).toBe(1);
    expect(decoded.sessions[0]).toMatchObject({ intakeSchemaVersion: 2, intakeRevision: 4 });
