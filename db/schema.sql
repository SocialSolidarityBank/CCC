...
--
-- 이 파일 = migrations/sqlite/0001_init.sql부터 0045_identity_revocation.sql까지를
--   순서대로 반영한 읽기용 누적 스냅샷이다.
...
-- ============================================================================
-- Migration 0040 — D74 independent institution roles and team supervision.
-- users.role remains the deployed identity compatibility field. Independent
-- human roles and team-scoped supervision live in the tables below.
-- ============================================================================
...
BEGIN SELECT RAISE(ABORT, 'authorization_role_assignment_immutable'); END;
...
BEGIN SELECT RAISE(ABORT, 'authorization_role_assignments_are_append_only'); END;
...
BEGIN
  ...
  SELECT lower(hex(randomblob(16))), NEW.org_id, NEW.id, 'institution_admin', 'legacy', NULL, NEW.created_at
  WHERE NEW.role = 'admin';
  INSERT INTO user_role_assignments (id, org_id, user_id, role, source, granted_by, granted_at)
  SELECT lower(hex(randomblob(16))), NEW.org_id, NEW.id, 'institution_technical_admin', 'legacy', NULL, NEW.created_at
  WHERE NEW.role = 'admin';
  INSERT INTO user_role_assignments (id, org_id, user_id, role, source, granted_by, granted_at)
  SELECT lower(hex(randomblob(16))), NEW.org_id, NEW.id, 'practitioner', 'legacy', NULL, NEW.created_at
...
BEGIN
  ...
  INSERT INTO user_role_assignments (id, org_id, user_id, role, source, granted_by, granted_at)
  SELECT lower(hex(randomblob(16))), NEW.org_id, NEW.id, 'institution_admin', 'legacy', NULL, datetime('now')
  WHERE NEW.role = 'admin';
  INSERT INTO user_role_assignments (id, org_id, user_id, role, source, granted_by, granted_at)
  SELECT lower(hex(randomblob(16))), NEW.org_id, NEW.id, 'institution_technical_admin', 'legacy', NULL, datetime('now')
  WHERE NEW.role = 'admin';
  INSERT INTO user_role_assignments (id, org_id, user_id, role, source, granted_by, granted_at)
  SELECT lower(hex(randomblob(16))), NEW.org_id, NEW.id, 'practitioner', 'legacy', NULL, datetime('now')
  WHERE NEW.role = 'counselor';
END;
...
BEGIN
  ...
  WHERE EXISTS (
    ...
    WHERE org_id = OLD.org_id AND user_id = OLD.id
      AND role = 'institution_admin' AND revoked_at IS NULL
  )
    AND NOT EXISTS (
      SELECT 1
      FROM user_role_assignments AS replacement
  ...
  WHERE EXISTS (
    SELECT 1 FROM user_role_assignments
    WHERE org_id = OLD.org_id AND user_id = OLD.id
      AND role = 'institution_technical_admin' AND revoked_at IS NULL
  )
    AND NOT EXISTS (
      SELECT 1
      FROM user_role_assignments AS replacement
...
BEGIN SELECT RAISE(ABORT, 'authorization_team_immutable'); END;
...
BEGIN SELECT RAISE(ABORT, 'authorization_teams_are_append_only'); END;
...
CREATE UNIQUE INDEX uq_team_memberships_active
  ON team_memberships (org_id, team_id, user_id) WHERE ended_at IS NULL;
CREATE INDEX idx_team_memberships_user
  ON team_memberships (org_id, user_id, team_id) WHERE ended_at IS NULL;
...
BEGIN
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM teams
    WHERE id = NEW.team_id AND org_id = NEW.org_id AND archived_at IS NULL
  );
  SELECT RAISE(ABORT, 'authorization_scope_violation')
  WHERE NOT EXISTS (
    SELECT 1
...
BEFORE UPDATE OF id, org_id, team_id, user_id, added_by, joined_at ON team_memberships
BEGIN SELECT RAISE(ABORT, 'authorization_team_membership_immutable'); END;
...
BEGIN SELECT RAISE(ABORT, 'authorization_team_memberships_are_append_only'); END;
...
BEGIN SELECT RAISE(ABORT, 'authorization_supervisor_grant_immutable'); END;
...
BEGIN SELECT RAISE(ABORT, 'authorization_supervisor_grants_are_append_only'); END;
...
-- ============================================================================
-- 0043: 개인정보 동의 문안 버전·해시·오프라인 원본 참조
-- ============================================================================
