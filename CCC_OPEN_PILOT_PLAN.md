...
| E3-8 동의 6영역 migration | E3-4, E3-5, SG7 | CCC-221의 PostgreSQL `0006_rls_default_deny.sql`과 SQLite `0050_rls_scope.sql` 뒤에 consent와 legacy observation을 한 paired migration으로 추가한다. 물리 번호는 구현 시점의 다음 번호를 쓰고 과거 예약 번호를 재사용하지 않는다. 6개 literal과 append-only event, parity, RLS, 기존 `ccc_api` role의 최소 grant와 schema snapshot을 갱신하고 기존 2종을 자동 승격하지 않는다. |
...
| E5-6 원음 생명주기와 삭제 | E0-5a, E1-3, E3-8, SG8 | D85의 경로별 동의 입장 조건과 시계로 E3-8 뒤에 audio objects paired migration과 parity를 추가한다. 물리 번호는 구현 시점의 다음 번호를 쓰고 이미 적용된 SQLite `0050`과 PostgreSQL `0006`을 재사용하지 않는다. 로컬 STT는 녹음 동의, Azure STT는 녹음과 외부 STT 동의를 요구한다. `retention_hard_cap_at=uploaded_at+7일`, 첫 처리 기회의 `processing_deadline_at=min(기회+24시간, 상한)`, 처리·동의 철회·불가 확정·상한 삭제, 관리자 사고, 삭제 증거 쓰기 실패를 멱등 조정한다. provider URL 세부 구현은 E6-3이 소유한다. |
